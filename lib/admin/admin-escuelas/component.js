...
export default class AdminEscuelas extends Component {
  ...
  showPanel = (event) => {
    const element =  event.target.tagName === 'SPAN' ? event.target.parentNode : event.target
    ...
    element.children[0].classList.toggle('activeAngle');
    element.nextSibling.classList.toggle('hiden')
