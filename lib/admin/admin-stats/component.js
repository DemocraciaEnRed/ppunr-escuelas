...
export default class AdminStats extends Component {
  ...
  render () {
    ...
    let { users, userNotEmailValidated, topics, likes, attendies, comments,
      ...
      usersWhoDidntVotedCount, isLoadingVotacion, votosOnline,
      votosPresencial, escuelas, claustros, showFakeDeleteVotes, showTrueDeleteVotes } = this.state

    let escuelasRows = () => {
      let rows = []
      rows.push(
        <tr>
          <td>-- Votos por escuela...</td>
          <td className="text-center">-</td>
        </tr>
      )
      for (let escuela in escuelas) {
        rows.push(
          <tr key={`escuela-${escuela}`}>
            <th>Personas de {escuela} que votaron</th>
            <td className="text-center">{escuelas[escuela]}</td>
          </tr>
        )
      }
      return rows
    ...
    return (
      <div>
        <h2>Stats</h2>
        <hr />
        <p className='h3'><i className="bar-chart"></i> Usuarios {isLoading && <span className="text-info">(Cargando)</span>}</p>
        ...
        <p className='h3'><i className="bar-chart"></i> Contenido {isLoading && <span className="text-info">(Cargando)</span>}</p>
        <table className='table table-bordered'>
          <tbody>
            <tr>
              <th>Proyectos</th>
        ...
        <p className='h3'><i className="bar-chart"></i> Agenda {isLoading && <span className="text-info">(Cargando)</span>}</p>
        <table className='table table-bordered'>
          <tbody>
            <tr>
