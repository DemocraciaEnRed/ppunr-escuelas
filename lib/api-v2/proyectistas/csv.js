...
function getEscuelasNombre (escuelasArray) {
  let aux = escuelasArray.map((escuela) => escuela.nombre)
  return aux.join(', ')
}
function getEscuelasAbreviacion (escuelasArray) {
  let aux = escuelasArray.map((escuela) => escuela.abreviacion)
  return aux.join(', ')
}
  ...
  function sendCsv (req, res, next) {
    var infoProyectistas = req.proyectistas.map((u) => {
      ...
      return [
        ...
        u.escuelas && u.escuelas.length > 0 ? escapeTxt(getEscuelasAbreviacion(u.escuelas)) : '??'
      ]
    })
    var data = [['Nombre', 'Apellido', 'DNI', 'Email', 'Claustro', 'Escuelas']]
