...
app.get('/padron/all/csv',
  ...
  function sendCsv (req, res, next) {    
    var infoPadron = req.data.map((p) => {
      ...
      return [
        p.dni,
        p.escuelas.map((e) => e.abreviacion).join('-'),
    ...
    var data = [['DNI', 'Escuelas', 'Nombre', 'Apellido', 'Email', 'Claustro']]
    ...
    json2csv(data, function (err, csv) {
      ...
      res.set({
        ...
        'Content-Disposition': `attachment; filename=padron-ppunre-${Math.floor((new Date()) / 1000)}.csv`
      })
      res.write(csv)
      res.end()
