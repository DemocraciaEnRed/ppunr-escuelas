...
exports.vote = function vote (opts) {
  ...
  return find()
    ...
    .populate(scopes.ordinary.populate)
    .exec()
    .then(doVote.bind(null, user, dni, value, claustro, votacionAbierta))
    .then((topic) => scopes.ordinary.expose(topic, forum, user))
}
...
function doVote (user, dni, value, claustro, votacionAbierta, topic) {
  ...
  return votedBy(user, topic, dni)
    .then((voted) => {
     ...
     if (value == 'voto'){
       ...
       if ((!user.staff) && ( !user.escuelas || !user.escuelas.find(escuelaId => escuelaId.toString() == topicEscuelaId))){
