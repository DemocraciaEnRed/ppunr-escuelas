...
exports.getProyectistas = () => {
  ...
  return User.find({ proyectista: true }).populate('claustro').populate('escuelas').then((users) => {
    if (!users)
...
exports.findByDNIWithAll = (dni) => {
  log('Looking for user with dni %s', dni)
  return User.findOne({ dni: dni }).populate('claustro escuelas')
}

exports.getAllUsersInDNIArray = (dniArray) => {
  log('Looking for users with dni %j', dniArray)
  return User.find({ dni: { $in: dniArray }, emailValidated: true }).populate('claustro escuelas')
}


exports.getUsersNotEmailValidated = () => {
  log('Looking for users with emailValidated = false')
  return User.find({ emailValidated: false }).populate('claustro').populate('escuelas')
}
...
exports.getUsersWhoDidntVotedWithClaustroAndEscuelas = (dniArray) => {
  log('Looking for users who didnt voted')
  return User.find({ dni: { $nin: dniArray }, emailValidated: true }).populate('claustro').populate('escuelas')
}
